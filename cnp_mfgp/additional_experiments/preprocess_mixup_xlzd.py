#!/usr/bin/env python3
"""Apply mixup augmentation to XLZD HDF5 training files.
...
2) Set `use_data_augmentation: mixup` in `src/xlzd/settings.yaml`.
3) Run the clean CNP training script or notebook.
"""
...
def mixup_one_file(
    ...
    with h5py.File(file_path, "a") as h5:
        ...
        target_headers = decode_labels(np.asarray(h5["target_headers"])) if "target_headers" in h5 else ["inside_theta"]
...
def load_settings(config_path: Path) -> Tuple[Path, List[float], List[str]]:
    ...
    signal_condition = cfg.get("simulation_settings", {}).get("signal_condition", ["inside_theta==1"])
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Clean mixup preprocessing for XLZD H5 training files")
    default_config = Path(__file__).resolve().parents[1] / "xlzd" / "settings.yaml"
    p.add_argument("--config", type=Path, default=default_config, help="Path to XLZD settings.yaml")
