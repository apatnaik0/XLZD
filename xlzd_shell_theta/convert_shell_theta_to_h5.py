#!/usr/bin/env python3
"""Convert shell-theta CSV/parquet files into HDF5 for the generic CNP pipeline."""
...
THETA_COLS = ["r_shell", "z_shell"]
PHI_COLS = ["s_r", "s_z_from_center"]
TARGET_COLS = ["near_shell"]
WEIGHTS_COL = "weights"
...
def write_h5(h5_path: Path, theta: np.ndarray, phi: np.ndarray, target: np.ndarray, weights: np.ndarray, fidelity: np.ndarray) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Convert shell-theta CSV/parquet files to HDF5")
    parser.add_argument(
        "--dataset-root",
        type=Path,
        default=repo_root / "outputs_shell_theta",
...
def main() -> None:
    ...
    print("=" * 72)
    print("CSV/PARQUET -> HDF5 conversion (XLZD shell theta)")
