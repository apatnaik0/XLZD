...
Target:
- inside_shell = 1 if an event lies within that shell band
"""
...
def infer_z_center(df: pd.DataFrame, shell_cfg: ShellConfig) -> float:
...
def build_shell_candidate_table(df_for_support: pd.DataFrame, shell_cfg: ShellConfig) -> pd.DataFrame:
    ...
    if out.empty:
        raise RuntimeError(
            "No valid shell candidates were found. Lower min_candidate_events or reduce n_shells."
        )
...
def main() -> None:
    ...
    candidates_path = save_dataframe(candidates_df, output_dir / "shell_candidates", output_format)
    manifest_df = pd.concat([lf_manifest, hf_manifest, val_manifest], ignore_index=True)
    manifest_path = save_dataframe(manifest_df, output_dir / "shell_candidates", output_format)
